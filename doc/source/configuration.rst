...
**optional**
  ...
  ``ignore-provider-quota``
    ...
    ``max-cores``, ``max-servers`` and ``max-ram`` for this pool and the
    current usage based on stored data. This may be useful in circumstances
    where the provider is incorrectly calculating quota.
