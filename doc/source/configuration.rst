...
to the credentials used by nodepool to login to the Jenkins master.

If the `credentials-id` key is provided, is used by the configured Jenkins
master to login to the Jenkins slaves. This id is a Jenkins credential
