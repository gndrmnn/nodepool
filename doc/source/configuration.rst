...
**one of**
  ...
  ``cloud-image``
    Refers to the name of an externally managed image in the cloud that is
    already existing provider. The value of ``cloud-image`` should match the
    ``name`` of a previously configured entry from the ``cloud-images`` section
    on the provider. See :ref:`provider_cloud_images`.
