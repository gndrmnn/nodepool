...
The `name` and `image` keys are required.  The `min-ready` key is
optional and defaults to 2. If the value is -1 nodepool will not
create a snapshot of the image. The `providers` list is also required
required if any nodes should actually be create (e.g., the label is
