...
Nodepool reads its secrets from ``/etc/nodepool/nodepool.conf``
by default. The secrets file is a standard ini config file, with
one section for mysql, and another section for the jenkins
