...
``targets`` sections. If building images using diskimage-builder,
also the ``diskimages`` section is required::
...
on this configuration. If no image matching is found, image
...
diskimages will be used.
See :ref:`scripts` for setup script details.
