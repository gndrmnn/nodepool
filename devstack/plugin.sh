...
function install_shade {
    if use_library_from_git "shade"; then
        ...
        $NODEPOOL_INSTALL/bin/pip3 install $DEST/shade
    fi
}
...
function install_diskimage_builder {
    if use_library_from_git "diskimage-builder"; then
        ...
        $NODEPOOL_INSTALL/bin/pip3 install $DEST/diskimage-builder
    fi
}
...
function install_glean {
    if use_library_from_git "glean"; then
        ...
        setup_dev_lib "glean"
        $NODEPOOL_INSTALL/bin/pip3 install $DEST/glean
    fi
}
...
function install_nodepool {
    ...
    $NODEPOOL_INSTALL/bin/pip3 install $DEST/nodepool
}
