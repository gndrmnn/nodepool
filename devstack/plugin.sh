...
EOF
    ...
    sudo chmod a+x \
        $(dirname $NODEPOOL_CONFIG)/elements/nodepool-setup/install.d/01-nodepool-setup
    sudo mkdir -p $DIB_BASE_PATH/images
    sudo mkdir -p $DIB_BASE_PATH/tmp
    sudo mkdir -p $DIB_BASE_PATH/cache
    sudo chown -R stack:stack $DIB_BASE_PATH
}
...
images-dir: $DIB_BASE_PATH/images
...
diskimages:
  - name: ubuntu-dib
    ...
    release: trusty
    env-vars:
      TMPDIR: $DIB_BASE_PATH/tmp
      DIB_IMAGE_CACHE: $DIB_BASE_PATH/cache
