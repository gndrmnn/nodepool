...
function start_nodepool {
    ...
    run_process geard "$NODEPOOL_HOME/bin/geard -p 8991 -d"
    ...
    run_process nodepool "$NODEPOOL_HOME/bin/nodepoold --no-builder -c $NODEPOOL_CONFIG -s $NODEPOOL_SECURE -l $NODEPOOL_LOGGING -d"
    run_process nodepool-builder "$NODEPOOL_HOME/bin/nodepool-builder -c $NODEPOOL_CONFIG -d"
    :
}
