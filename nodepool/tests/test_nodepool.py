...
class TestNodepool(tests.DBTestCase):
    ...
    def test_proxy_timeout(self):
        ...
        task = nodepool.task_manager.Task()
        task.run = mock.Mock(side_effect=_side_effect)

        manager.submitTask(task)

        self.assertEqual(task.run.call_count, 2)
