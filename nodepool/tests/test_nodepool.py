...
class TestNodepool(tests.DBTestCase):
    ...
    def test_image_delete(self):
        ...
        images = self.zk.getBuilds('fake-image')
        self.assertEqual(len(images), 1)
        build_number = images[0].id
        self.assertEqual(build_number, '0000000001')
        self.zk.deleteBuild('fake-image', build_number)

        while True:
            images = self.zk.getBuilds('fake-image')
            if build_number not in [x.id for x in images]:
                break
            time.sleep(.1)
