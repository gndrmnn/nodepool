...
class TestNodepool(tests.DBTestCase):
    ...
    def test_dib_upload_multi_provider_and_images(self):
        ...
        pool.start()
        for count in range(1, len(results)):
            self.waitForImage(
                pool, results[count - 1]['provider'],
                results[count -1]['image'])
        self.waitForNodes(pool)

        with pool.getDB().getSession() as session:
#            for count in range(1, len(results)):
#                image = session.getSnapshotImage(count)
#                self.assertEqual(image.image_name, results[count - 1]['image'])
#                self.assertEqual(
#                    image.provider_name, results[count -1]['provider'])
            image = session.getSnapshotImage(1)
            self.assertEqual(image.image_name, results[0]['image'])
            self.assertEqual(
                image.provider_name, results[0]['provider'])
            image = session.getSnapshotImage(2)
            self.assertEqual(image.image_name, results[1]['image'])
            self.assertEqual(
                image.provider_name, results[1]['provider'])
            image = session.getSnapshotImage(3)
            self.assertEqual(image.image_name, results[2]['image'])
            self.assertEqual(
                image.provider_name, results[2]['provider'])
            image = session.getSnapshotImage(4)
            self.assertEqual(image.image_name, results[3]['image'])
            self.assertEqual(
                image.provider_name, results[3]['provider'])
