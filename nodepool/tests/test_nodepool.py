...
class TestNodepool(tests.DBTestCase):
    ...
    def test_leaked_node_with_launcher_id(self):
        self._test_leaked_node('leaked_node_launcher_id.yaml')
    ...
    def test_leaked_node_not_deleted(self):
        ...
        configfile = self.setup_config('leaked_node_launcher_id.yaml')
        pool = self.useNodepool(configfile, watermark_sleep=1)
        self._useBuilder(configfile)
        pool.start()
        self.waitForImage('fake-provider', 'fake-image')
