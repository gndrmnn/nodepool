...
class TestNodepool(tests.DBTestCase):
    ...
    def test_node_ipv6(self):
        """Test that a node is created w/ or w/o force-ipv4 flag"""
        ...
        self.waitForImage('fake-provider4', 'fake-image')
        ...
        label4_nodes = self.waitForNodes('fake-label4')

        self.assertEqual(len(label1_nodes), 1)
        self.assertEqual(len(label2_nodes), 1)
        self.assertEqual(len(label3_nodes), 1)
        self.assertEqual(len(label4_nodes), 1)

        # force-ipv4 false and ipv6 address available
        ...
        # force-ipv4 false but ipv6 address unavailable
        ...
        # force-ipv4 set to true and no ipv6 address available
        ...
        self.assertEqual(label3_nodes[0].interface_ip, 'fake')

        # force-ipv4 set to true and ipv6 address available
        self.assertEqual(label4_nodes[0].provider, 'fake-provider4')
        self.assertEqual(label4_nodes[0].public_ipv4, 'fake')
        self.assertEqual(label4_nodes[0].public_ipv6, 'fake_v6')
        self.assertEqual(label4_nodes[0].interface_ip, 'fake')
