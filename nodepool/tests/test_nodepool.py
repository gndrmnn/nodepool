...
class TestNodepool(tests.DBTestCase):
    ...
    def test_two_labels_min_ready_disabled(self):
        configfile = self.setup_config(
            'node_two_labels_min_ready_disabled.yaml')
        pool = self.useNodepool(configfile, watermark_sleep=1)
        self._useBuilder(configfile)
        pool.start()
        self.waitForImage('fake-provider', 'fake-image')
    ...
    def test_two_provider_max_servers_disabled(self):
        configfile = self.setup_config(
            'node_two_provider_max_servers_disabled.yaml')
        pool = self.useNodepool(configfile, watermark_sleep=1)
        self._useBuilder(configfile)
        pool.start()
        self.waitForImage('fake-provider', 'fake-image')
