...
from nodepool import builder, exceptions, fakeprovider, tests, zk
...
class TestNodePoolBuilder(tests.DBTestCase):
    ...
    def test_image_upload_fail(self):
        ...
        # Request an image build, forcing a reupload
        current = self.zk.getMostRecentImageUpload('fake-image',
                                                   'fake-provider')
        self.zk.submitBuildRequest('fake-image')
        self.waitForImage('fake-provider', 'fake-image', ignore_list=[current])
        ...
        self.assertNotEqual(0, len(uploads))
