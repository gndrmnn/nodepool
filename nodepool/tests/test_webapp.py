...
class TestWebApp(tests.DBTestCase):
    ...
    def _test_alien_image_list_json(self, conf):
        ...
        req = request.Request(
            "http://localhost:%s/alien-image-list" % port)
        req.add_header('Accept', 'application/json')
        f = request.urlopen(req)
        self.assertEqual(f.info().get('Content-Type'),
                         'application/json')
    ...
    def test_info_json(self):
        ...
        req = request.Request(
            "http://localhost:%s/info/builds?provider=fake-provider" % port)
        req.add_header('Accept', 'application/json')
        f = request.urlopen(req)
        self.assertEqual(f.info().get('Content-Type'),
                         'application/json')
        ...
        req = request.Request(
            "http://localhost:%s/info/nodes?provider=fake-provider" % port)
        req.add_header('Accept', 'application/json')
        f = request.urlopen(req)
        self.assertEqual(f.info().get('Content-Type'),
                         'application/json')
        ...
        for node in p1_nodes:
            self.assertTrue(any(node.id == n['id'] for n in objs))
