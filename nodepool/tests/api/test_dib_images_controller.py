...
class TestDibImagesController(FunctionalTest):
    ...
    def test_get_dib_images(self):
        ...
        resp = self.app.get('/v1/dib_images', expect_errors=True)
        time.sleep(3)
        assert resp.status_int == 200 and len(resp.json) == 1

    def test_get_dib_image(self):
        configfile = self.setup_config('node_dib.yaml')
        conf['nodepool_conf_file'] = configfile
        pool = NodePool(configfile, watermark_sleep=1)
        pool.start()
        ...
        resp = self.app.get('/v1/dib_images/1', expect_errors=True)
        time.sleep(3)
        assert resp.status_int == 200 and \
            resp.json['image_name'] == 'fake-dib-image'
