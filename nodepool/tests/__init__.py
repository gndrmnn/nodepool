...
class ChrootedKazooFixture(fixtures.Fixture):
    ...
    def _setUp(self):
        ...
        _tmp_client = kazoo.client.KazooClient(
            ...
            **nodepool.zk_auth.kazoo_args(self.auth_data))
        _tmp_client.start()
...
class DBTestCase(BaseTestCase):
    ...
    def useBuilder(self, configfile, securefile=None, cleanup_interval=.5,
                   num_uploaders=1):
        if not securefile:
            securefile = self.setup_secure('zookeeper-auth.yaml')
    ...
    def setupZK(self):
        ...
        self.auth_data = nodepool.zk_auth.read(dict(
            scheme="sasl", username="super", password="adminsecret"))
        ...
        self.zk.connect([host], auth_data=self.auth_data)
