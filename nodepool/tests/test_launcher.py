...
class TestLauncher(tests.DBTestCase):
    ...
    def test_ignore_quota(self):
        ...
        # Set ignore-quota to True and try the same request, but
        # this time have no max-servers, max-cores or max-ram set.
        # This should fail to protect someone from having absolutely
        # no limits and crashing their cloud due to them hitting a
        # real limit without realising it.
        self.log.debug("Using ignore-quota True, with no other limits")
        self.replace_config(configfile, 'ignore_quota_true_nolimits.yaml')
