...
class TestLauncher(tests.DBTestCase):
    ...
    def test_multiple_launcher(self):
        ...
        # TODO(pabelanger): While this is the current behavior of nodepool, we
        # like to change it in the future. This means, for each launcher
        # online, we could have upto min-ready * nodepool-launcher online.
        ...
        for node in nodes:
            if node.provider == 'fake-provider2':
                ...
                provider1 = True
            self.assertEqual(node.type, 'fake-label')
            self.assertEqual(node.username, 'zuul')
            self.assertNotEqual(node.host_keys, [])
        self.assertTrue(provider1)
        self.assertTrue(provider2)
