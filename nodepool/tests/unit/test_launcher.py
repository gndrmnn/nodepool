...
class TestLauncher(tests.DBTestCase):
    ...
    def test_decline_from_quota_false(self):
        ...
        def fake_get_quota():
            nonlocal cpu_quota
            return (cpu_quota, 20, 1000000)

        def release_request(req):
            nonlocal cpu_quota
            nonlocal pool
            # Bump up and invalidate the quota to allow the request to be
            # fulfilled
            cpu_quota = 4
            pm = pool.getProviderManager('fake-provider')
            pm.invalidateQuotaCache()
            req = self.waitForNodeRequest(req, zk.FULFILLED)

            # Mark the node as used to free the request
            req_node = self.zk.getNode(req.nodes[0])
            self.zk.lockNode(req_node, blocking=False)

            self.log.debug("Marking node as used %s", req.id)
            req_node.state = zk.USED
            self.zk.storeNode(req_node)
            self.zk.unlockNode(req_node)
            self.waitForNodeDeletion(req_node)

            # Drop CPU quota again to block subsequent requests
            cpu_quota = 0
            pm.invalidateQuotaCache()
        ...
        # Request a 2 node set; this request should fail
        # due to the provider only being able to fulfill
        # a single node at a time (max-servers = 1)
        req3 = zk.NodeRequest()
        req3.state = zk.REQUESTED
        req3.node_types.append('fake-label')
        req3.node_types.append('fake-label')
