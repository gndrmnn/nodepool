...
class TestLauncher(tests.DBTestCase):
    ...
    def test_ignore_provider_quota_true_with_provider_error(self):
        ...
        # This should timeout since the request will continue to be re-issued
        # to the provider as long as the provider returns a quota error
        with self.assertRaisesRegex(Exception, r"Timeout waiting"):
            req = self.waitForNodeRequest(req)
