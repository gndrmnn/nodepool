...
class TestDriverKubernetes(tests.DBTestCase):
    ...
    def setUp(self):
        ...
        self.useFixture(
            fixtures.MockPatch(
                "nodepool.driver.kubernetes.provider.get_client", fake_get_client
            )
        )
    ...
    def test_kubernetes_machine(self):
        ...
        self.assertEqual(node.attributes, {"key1": "value1", "key2": "value2"})
                ...
                "containers": [
                    {
                        ...
                        "volumeMounts": [
                            {"mountPath": "/data", "name": "my-csi-inline-vol"}
                        ],
                    }
                ],
    ...
    def test_kubernetes_default_label_resources(self):
        ...
        self.assertEqual(
            ...
            {
                "limits": {"cpu": 2, "ephemeral-storage": "10M", "memory": "1024Mi"},
                "requests": {"cpu": 2, "ephemeral-storage": "10M", "memory": "1024Mi"},
            },
        )
        ...
        self.assertEqual(
            pod["spec"]["containers"][0]["resources"],
            {
                "limits": {"cpu": 4, "ephemeral-storage": "10M", "memory": "1024Mi"},
                "requests": {"cpu": 4, "ephemeral-storage": "10M", "memory": "1024Mi"},
            },
        )
        ...
        self.assertEqual(
            pod["spec"]["containers"][0]["resources"],
            {
                "limits": {"cpu": 2, "ephemeral-storage": "10M", "memory": "2048Mi"},
                "requests": {"cpu": 2, "ephemeral-storage": "10M", "memory": "2048Mi"},
            },
        )
        ...
        self.assertEqual(
            pod["spec"]["containers"][0]["resources"],
            {
                "limits": {"cpu": 2, "ephemeral-storage": "20M", "memory": "1024Mi"},
                "requests": {"cpu": 2, "ephemeral-storage": "20M", "memory": "1024Mi"},
            },
        )
        ...
        for node in (node_default, node_cust_cpu, node_cust_mem, node_cust_gpu):
            node.state = zk.DELETING
            self.zk.storeNode(node)
            self.waitForNodeDeletion(node)
    ...
    def test_kubernetes_default_label_limits(self):
        ...
        self.assertEqual(
            pod["spec"]["containers"][0]["resources"],
            {
                "limits": {"cpu": 8, "ephemeral-storage": "40M", "memory": "4196Mi"},
                "requests": {"cpu": 2, "ephemeral-storage": "10M", "memory": "1024Mi"},
            },
        )
        ...
        self.assertEqual(
            pod["spec"]["containers"][0]["resources"],
            {
                "limits": {"cpu": 4, "ephemeral-storage": "40M", "memory": "4196Mi"},
                "requests": {"cpu": 2, "ephemeral-storage": "10M", "memory": "1024Mi"},
            },
        )
        ...
        self.assertEqual(
            pod["spec"]["containers"][0]["resources"],
            {
                "limits": {"cpu": 8, "ephemeral-storage": "40M", "memory": "2048Mi"},
                "requests": {"cpu": 2, "ephemeral-storage": "10M", "memory": "1024Mi"},
            },
        )
        ...
        self.assertEqual(
            pod["spec"]["containers"][0]["resources"],
            {
                "limits": {"cpu": 8, "ephemeral-storage": "20M", "memory": "4196Mi"},
                "requests": {"cpu": 2, "ephemeral-storage": "10M", "memory": "1024Mi"},
            },
        )
    ...
    def test_kubernetes_tenant_quota_servers(self):
        self._test_kubernetes_quota("kubernetes-tenant-quota-servers.yaml", pause=False)

    def test_kubernetes_tenant_quota_cores(self):
        self._test_kubernetes_quota("kubernetes-tenant-quota-cores.yaml", pause=False)

    def test_kubernetes_tenant_quota_ram(self):
        self._test_kubernetes_quota("kubernetes-tenant-quota-ram.yaml", pause=False)

    def test_kubernetes_tenant_quota_extra(self):
        self._test_kubernetes_quota("kubernetes-tenant-quota-extra.yaml", pause=False)
    ...
    def test_node_list_k8s_json(self):
        ...
        req = request.Request("http://localhost:%s/node-list" % port)
        req.add_header("Accept", "application/json")
        f = request.urlopen(req)
        self.assertEqual(f.info().get("Content-Type"), "application/json")
        data = f.read()
