...
def loadConfig(config_path):
    ...
    for provider in config.get('providers', []):
        ...
        for pool in provider.get('pools', []):
            ...
            pp.provider = p
            p.pools[pp.name] = pp
            # Default the max values with math.nan to ease calculations
            # with unlimited quota.
            pp.max_cores = pool.get('max-cores', math.nan)
            pp.max_servers = pool.get('max-servers', math.nan)
            pp.max_ram = pool.get('max-ram', math.nan)
