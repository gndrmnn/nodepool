...
class NodePoolBuilder(object):
    ...
    def _unregister_image_id(self, worker, image_id):
        if image_id in self._buit_image_ids:
            self.log.debug('unregistering image %d', image_id)
            worker.unRegisterFunctoin('image-upload:%s' % image_id)
            self._buit_image_ids.remove(image_id)
        else:
    ...
    def _delete_image(self, image_id):
        self._unregister_image_id(image_id)
