...
class BuildWorker(BaseWorker):
    ...
    def _checkForScheduledImageUpdates(self):
        ...
        for name, image in self._config.diskimages.items():
            ...
            if not build or (now - build['state_time']) >= image.rebuild_age:
                # No build data recorded for this image, or it has aged out.
                try:
    ...
    def _checkForCleanup(self):
        ...
        for image in self._config.diskimages.keys():
            chosen = self._zk.getBuildsWithStates(image, ['delete', 'unused'])
            if not chosen:
                continue
            ...
            for build_num, data in chosen.items():
                ...
                if state == 'delete' and builder == self._hostname:
                    self.log.info(
                        "Deleting local build %s for image" % (build_num,
                                                               image))
                    self._deleteLocalBuild()
                    self._zk.deleteBuild(image, build_num)
                elif state == 'unused':
                    self.log.info(
                        "Deleting data for build %s for image %s" % (build_num,
                                                                     image))
                    self._zk.deleteBuild(image, build_num)
            ...
            if fail_list:
                for bnum, data in fail_list:
                    ...
                    if (now - data['state_time']) > IMAGE_CLEANUP:
                        ...
                        try:
                            with self._zk.imageBuildLock(
                                image, blocking=False
                            ):
                                self.log.info(
                                    "Marking %s:%s for delete" % (image, bnum))
                                self._zk.storeBuild(image, bnum, data)
                        except exceptions.ZKLockException:
                            pass

            #--------------------------------------------------------------
            # Mark ready builds past expiration time for deletion, keeping
            # the current and previous builds, no matter how old.
            #--------------------------------------------------------------
            ready_list = self._zk.getBuildsWithStates(image, ['ready'])

            # Skip if no ready images or only have a current and previous
                ...
                if (now - data['state_time']) > IMAGE_CLEANUP:
                    ...
                    try:
                        with self._zk.imageBuildLock(image, blocking=False):
                            ...
                            self._zk.storeBuild(image, key, data)
                    except exceptions.ZKLockException:
                        pass
