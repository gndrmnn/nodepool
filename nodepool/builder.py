        ...
        if upload.state == zk.DELETING:
            ...
            try:
                with self._zk.imageUploadNumberLock(upload, blocking=False):
                    ...
                    # In order to do a proper back-off we need to update the
                    # image upload and check the delete_attempts and age.
                    upload = self._zk.getImageUpload(
                        upload.image_name, upload.build_id,
                        upload.provider_name, upload.id)

                    # Check if we should delete the image now. The backoff is
                    # calculated by 2 ^ delete_attempts times the cleanup
                    # interval with a max of one hour.
                    delete_attempts = upload.delete_attemps or 0
                    min_age = min(2**delete_attempts * self._interval, 3600)
                    age = time.time() - upload.state_time

                    if age < min_age:
                        self.log.debug("Upload delete backoff, not deleting "
                                       "image build %s from %s", base,
                                       upload.provider_name)
                        return

                    try:
                        # It is possible we got this far, but don't actually
                        # have an external_name. This could mean that zookeeper
                        # and cloud provider are some how out of sync.
