...
class CleanupWorker(BaseWorker):
    ...
    def _isRecentUpload(image, provider, build_id, upload_id):
        '''
        ...
        for image in image_names:
            ...
            for build_id, build_data in all_builds:
                if build_data['state'] != 'deleted':
                    if build_id in builds_to_keep.keys():
                        continue

                # Check builds to keep based on what is uploaded to the provider.
                # This will have to span across ALL builds.
    ...
    def run(self):
        ...
        while self._running:
            ...
            cleanup_timeout = os.getenv('NODEPOOL_CLEANUP_TIMEOUT', 60)
