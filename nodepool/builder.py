    ...
    def _uploadImage(self, build_id, upload_id, image_name, images, provider,
        ...
        if provider.upload_script:
            try:
                ...
                self.log.info('Running upload script %s', cmd)
                p = subprocess.run(cmd, stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE, check=True)
                upload_script_output = p.stdout.decode()

                # parse image id from script output
                ids = re.findall("Image ID: ([\w-]+)", upload_script_output)
                if not ids:
                    raise Exception("Could not parse image id from upload script output")
                external_id = ids[0]

            except Exception as e:
                if isinstance(e, subprocess.CalledProcessError):
                    self.log.error('Upload script failed with '
                                   'exit code %s\nstdout:\n%s\nstderr:\n%s',
            ...
            self.log.info(
                'Upload script success with exit code %s\n'
                'stdout:\n%s\nstderr:\n%s',
