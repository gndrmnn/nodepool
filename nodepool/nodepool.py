        ...
        while self.running:
            ...
            try:
                if not self.paused:
                    ...
                    # satisify its assigned request, so we need to go through
                    # them and give them another shot. Unpause ourselves if
                    # they've all completed.
                    self.log.info("In paused state, draining handlers")
                    num_paused = 0
                    for handler in self.request_handlers:
                        if handler.paused:
                            ...
                            num_paused += 1
                    if num_paused == 0:
                        self.log.info("Unpausing, handlers drained")
                        self.paused = False
