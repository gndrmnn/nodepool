                    ...
                    if not meta:
                        self.log.debug("Instance %s (%s) in %s has no "
                                       "nodepool metadata" % (
                                           ...
                                           provider.name))
                        continue
                    if meta['nodepool_provider_name'] not in known_providers:
                        self.log.debug("Instance %s (%s) in %s "
                                       "lists unknown provider %s" % (
                                           server['name'], server['id'],
                                           provider.name,
                                           meta['nodepool_provider_name']))
                        continue
                    snap_image_id = meta.get('nodepool_snapshot_image_id')
                    node_id = meta.get('nodepool_node_id')
                    if snap_image_id:
