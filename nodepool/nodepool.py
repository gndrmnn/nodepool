...
class DiskImageBuilder(threading.Thread):
    ...
    def buildImage(self, image_id):
        with self.nodepool.getDB().getSession() as session:
            ...
            image_details = self.nodepool.config.diskimages[self.dib_image.image_name]
...
class NodePool(threading.Thread):
            ...
            if label.is_diskimage:
                ...
                for dib_image in session.getDibImages():
                    if (dib_image.image_name == label.image and
                        dib_image.state in [nodedb.READY, nodedb.BUILDING]):
                           found = True
                           break
                if not found:
        ...
        with self.getDB().getSession() as session:
            ...
            if queued_images:
                ...
                try:
                    ...
                    self.log.debug("Queued image building task for %s" % image.name)
    ...
    def cleanupOneDibmage(self, session, image):
        delete = False
        now = time.time()
        if (image.image_name not in
              self.config.labels[image.image_name]):
            delete = True
            self.log.info("Deleting image id: %s which has no current "
                          "base image" % image.id)
        else:
