...
class InstanceDeleter(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self._zk.unlockNode(self._node)
            return

        self.log.info("Deleting ZK node id=%s, state=%s, external_id=%s",
                      self._node.id, self._node.state, self._node.external_id)
        self._zk.unlockNode(self._node)
...
class NodeCleanupWorker(threading.Thread):
    ...
    def _deleteInstance(self, node):
        ...
        try:
            t = InstanceDeleter(self._zk, self._managers[node.provider], node)
            t.start()
        except Exception:
