...
class StatsReporter(object):
    ...
    Any other class that wishes to report information to statsd should
    ...
    def updateNodeStats(self, zk_conn, config_labels, config_providers):
        '''
        Refresh statistics for all known nodes across all providers.

        :param ZooKeeper zk_conn: A ZooKeeper connection object.
        :param dict labels: A dict of config Label objects.
        :param dict providers: A dict of config Provider objects.
        '''
        if not self._statsd:
            return
        ...
        for state in zk.Node.VALID_STATES:
            ...
            for label in config_labels.values():
                key = 'nodepool.label.%s.nodes.%s' % (label.name, state)
                states[key] = 0
            for provider in config_providers.values():
        ...
        for node in zk_conn.nodeIterator():
            ...
            key = 'nodepool.label.%s.nodes.%s' % (node.type, state)
            ...
            key = 'nodepool.provider.%s.nodes.%s' % (node.provider, state)
