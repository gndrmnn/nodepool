...
class NodeLaunchManager(object):
    ...
    def launch(self, node):
        ...
        t = NodeLauncher(node, self._retries)
        t.start()
...
class NodeRequestHandler(object):
    ...
    def _run(self):
        ...
        self.launch_manager = NodeLaunchManager(retries=3)
    ...
    def poll(self):
        ...
        # Launched nodes should be either READY (with allocated_to set) or
        # FAILED, so record them as such.
        for node in self.launch_manager.failed_nodes:
            self.zk.storeNode(node)
        for node in self.launch_manager.ready_nodes:
            self.zk.storeNode(node)
...
class ProviderWorker(threading.Thread):
    ...
    def stop(self):
        ...
        # Wait for the request handlers to finish.
        # TODO(Shrews): Maybe have a way to force them to stop?
        while self.request_handlers:
            self._removeCompletedHandlers()
            time.sleep(self.watermark_sleep)
