...
class NodePool(threading.Thread):
        ...
        for label in self._submittedRequests.keys():
            ...
            for req in label_requests:
                ...
                if req.state == zk.FULFILLED:
                    self.log.debug("min-ready node request fulfilled: %s", req)
            ...
            if active_requests:
                ...
                self.log.debug(
                    "min-ready node requests for %s complete", label)
    ...
    def createMinReady(self):
        ...
        import pprint
        self.log.debug("READY NODES: %s", pprint.pprint(ready_nodes))
