...
class NodeCleanupWorker(threading.Thread):
    ...
    def _cleanupLeakedInstances(self):
        ...
        for provider in self._nodepool.config.providers.values():
            ...
            known = [n.external_id for n in zk_conn.nodeIterator() if n.provider == provider.name]

            for server in servers:
                # We don't use meta other than to verify that this is a
                # nodepool created server.
