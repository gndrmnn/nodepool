        ...
        with self.nodepool.getDB().getSession() as session:
            ...
            try:
                ...
                self.log.exception("Exception launching subnode id: %s for "
                                   ...
                                   (self.subnode_id, self.node_id))
                try:
                    self.nodepool.deleteSubNode(self.subnode_id)
                except Exception:
...
class NodePool(threading.Thread):
    ...
    def getNeededNodes(self, session):
        ...
        for label in self.config.labels.values():
            min_ready = total_label_min_ready.get(label.name, 0)
            min_ready += label.min_ready
            total_label_min_ready[label.name] = min_ready
        ...
        def count_subnodes(provider_name):
            count = 0
        ...
        for label_name in total_label_min_ready:
            start_demand = label_demand.get(label_name, 0)
            min_demand = max(start_demand, total_label_min_ready[label_name])
            n_ready = count_nodes(label_name, nodedb.READY)
            n_building = count_nodes(label_name, nodedb.BUILDING)
            n_test = count_nodes(label_name, nodedb.TEST)
            ready = n_ready + n_building + n_test
            demand = max(min_demand - ready, 0)
            label_demand[label_name] = demand
            self.log.debug("  Deficit: %s: %s (start: %s min: %s ready: %s)" %
                           (label_name, demand, start_demand, min_demand,
                            ready))
        ...
        for provider in self.config.providers.values():
            ...
            n_provider = count_subnodes(provider.name)
