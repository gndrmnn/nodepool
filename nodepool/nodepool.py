...
class NodePool(threading.Thread):
    ...
    def removeFulfilledRequests(self):
        '''
        ...
        for label in self._submittedRequests.keys():
            ...
            if (self._submittedRequests[label] and
                self._submittedRequests[label].state == zk.FULFILLED
            ):
    ...
    def createMinReady(self):
        ...
        for label in self.config.labels.values():
            ...
            if need:
                self.log.info("Creating request for %d %s nodes", need, label)
