...
class NodeLauncher(threading.Thread, StatsReporter):
    ...
    def __init__(self, zk, provider_label, provider_manager, requestor,
        ...
        self._unmanagedimage = self._provider.unmanagedimages.get(self._label.image_name, None)
    ...
    def _launchNode(self):
        if self._label.diskimage:
            ...
            # launch using unmanaged image
            config_drive = self._unmanagedimage.config_drive
            image_external_id = self._label.image_name
            image_id = self._label.image_name
            image_name = self._unmanagedimage.name
...
class NodeRequestHandler(object):
    ...
    def _imagesAvailable(self):
        ...
        for label in self.request.node_types:
            ...
            if self.pool.labels[label].image_name:
                img = self.pool.labels[label].image_name
    ...
    def labelImageIsAvailable(self, label):
        ...
        for pool in label.pools:
            for pool_label in pool.labels.values():
                if pool_label.image_name:
                    manager = self.getProviderManager(pool.provider.name)
                    if manager.getImage(pool_label.image_name):
                        return True
