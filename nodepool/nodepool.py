...
class ImageUpdater(threading.Thread):
    ...
    def __init__(self, nodepool, provider, image_name, snap_image_id):
    ...
    def _run(self):
        with self.nodepool.getDB().getSession() as session:
            self.log.debug("Updating image %s in %s " % (self.image_name,
                                                         self.provider.name))
            try:
            ...
            try:
                ...
                self.log.exception("Exception updating image %s in %s:" %
                                   (self.image_name, self.provider.name))
                try:
                    if self.snap_image:
...
class DiskImageUpdater(ImageUpdater):
    ...
    def __init__(self, nodepool, provider, image_name, snap_image_id,
                 filename):
        super(DiskImageUpdater, self).__init__(nodepool, provider, image_name,
                                               snap_image_id)
...
class NodePool(threading.Thread):
    ...
    def getNeededSubNodes(self, session):
        ...
        for node in session.getNodes():
            if node.label_name in self.config.labels:
                ...
                # that node belonged to a label that does not exist
                # no need of subnodes
                return None
                ...
                for dib_image in session.getDibImages():
                    if (dib_image.image_name == label.image and
                        ...
                        if (dib_image.state == nodedb.READY and
                            not os.path.exists(dib_image.filename)):
    ...
    def uploadImage(self, session, provider, image_name):
        ...
        try:
            ...
            t = DiskImageUpdater(self, self.config.providers[provider],
                                 image_name, snap_image.id, filename)
            t.start()
