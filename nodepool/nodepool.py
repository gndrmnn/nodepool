...
class NodeCompleteThread(threading.Thread):
    ...
    def __init__(self, nodepool, job, args, hostname):
    ...
    def run(self):
        try:
            ...
            self.log.exception("Exception handling job for %s:" %
                               self.hostname)
            try:
                self.job.sendWorkFail()
            except Exception:
                self.log.exception("Further exception sending job failure:")

    def handleJob(self):
        node = self.db.getNodeByHostname(self.hostname)
        if not node:
            raise Exception("Unable to find node with hostname: %s" %
                            self.hostname)
        self.nodepool.deleteNode(node)
        self.job.sendWorkComplete()


class NodeUpdateWorker(threading.Thread):
    log = logging.getLogger("nodepool.NodeUpdateWorker")

    def __init__(self, nodepool):
        threading.Thread.__init__(self)
        w = gear.Worker('node_update')
        w.addServer('127.0.0.1', 4730)
        w.waitForServer()
        w.registerFunction("nodepool_update")
        self.worker = w
        self._stopped = False
        self.db = nodedb.NodeDatabase()
        self.nodepool = nodepool
        ...
        while not self._stopped:
            ...
            try:
                ...
                try:
                    ...
                    self.log.exception("Further exception sending job "
                                       "failure:")
    ...
    def handleJob(self, job):
        ...
        if args['phase'] == 'start':
            self.handleStartPhase(job, args, hostname)
        elif args['phase'] == 'complete':
            self.handleCompletePhase(job, args, hostname)
        else:
            raise Exception("Received job for unhandled phase: %s" %
                            args['phase'])

    def handleStartPhase(self, job, args, hostname):
        node = self.db.getNodeByHostname(hostname)
        if not node:
            raise Exception("Unable to find node with hostname: %s" %
                            hostname)
    ...
    def handleCompletePhase(self, job, args, hostname):
        t = NodeCompleteThread(self.nodepool, job, args, hostname)
        t.start()
...
class NodeLauncher(threading.Thread):
    ...
    def createJenkinsNode(self):
        ...
        name = self.node.hostname.split('.')[0]
...
class NodePool(threading.Thread):
    ...
    def __init__(self):
        threading.Thread.__init__(self)
        self.db = nodedb.NodeDatabase()
        self.update_worker = NodeUpdateWorker(self)
    ...
    def loadConfig(self):
        ...
        config = yaml.load(open('nodepool.yaml'))
    ...
    def deleteNode(self, node):
        ...
        if target.jenkins_url:
            ...
            jenkins_name = node.hostname.split('.')[0]
