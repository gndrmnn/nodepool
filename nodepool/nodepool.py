...
class NodePool(threading.Thread):
    ...
    def loadConfig(self):
        ...
        for provider in config['providers']:
            ...
            if oldclient:
                if (p.username != oldclient.user or
                    p.password != oldclient.password or
                    p.project_id != oldclient.projectid or
                    p.service_type != oldclient.service_type or
                    p.service_name != oldclient.service_name or
                    p.region_name != oldclient.region_name):
                    oldclient = None
            if oldclient:
