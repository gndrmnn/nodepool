...
class DiskImageBuilder(threading.Thread):
    ...
    def run(self):
        while True:
            # grabs image from queue
            image, filename = self.queue.get()
            self.buildImage(image, filename)
            self.queue.task_done()
            self.nodepool._images_in_queue.remove(image.name)

    def _buildImage(self, image, filename):
        if filename.startswith('./fake-dib-image'):
            return True

        # remove all previous images with that pattern
        items = self.nodepool._getStoredImages(image.name, True)
        if items:
            for item in items:
                # remove it
    ...
    def buildImage(self, image, filename):
        self.log.info("Creating image id: %s with filename %s" %
                      (image.name, filename))
        self._buildImage(image, filename)
...
class NodePool(threading.Thread):
    ...
    def checkForMissingImages(self, session):
        ...
        for label in self.config.labels.values():
            ...
            if label.is_diskimage:
                image_filename = self._getStoredImages(label.image)
                if not image_filename:
                    ...
                    for provider_name in label.providers:
                        ...
                        if not found:
                            ...
                            self.uploadImage(session, provider_name, label)
            else:
                # snapshots
                for provider_name in label.providers:
                    found = False
    ...
    def updateImages(self, session):
        ...
        for label in self.config.labels.values():
            ...
            if label.image.is_diskimage:
                self.buildImage(self.pool.config.diskimages[image.image])
                needs_build = True
        if needs_build:
        ...
        for label in self.config.labels.values():
            ...
            for provider in label.providers:
                if label.image.is_diskimage:
                    self.uploadImage(session, providre.name, label.image.name)
                else:
                    self.updateImage(session, provider.name, label.image.name)
    ...
    def _updateImage(self, session, provider_name, image_name):
        ...
        if not image_label.is_diskimage:
            ...
            raise Exception(
                "Cannot update disk image images. Please build and upload images")


    def buildImage(self, image):
        # check if we already have this item in the queue
        if image.name in self._images_in_queue:
            self.log.exception('Image %s is already being built' % image.name)
        else:
            try:
                start_time = time.time()
                timestamp = int(start_time)
                ...
                filename = os.path.join(self.config.imagesdir,
                                        '%s-%s' % (image.name, str(timestamp)))

                # add this build to queue
                self._image_builder_queue.put([image, filename])
                self._images_in_queue.add(image.name)
                return filename
            except Exception:
                self.log.exception(
                    "Could not build image %s", image.name)

    def _getStoredImages(self, image, include_directories=False):
        if include_directories:
            pattern_fragment = "[qcow2|d]"
        else:
            pattern_fragment = "qcow2"
        pattern = '%s/%s-*[0-9].*%s' % (self.config.imagesdir, image,
                                        pattern_fragment)
    ...
    def uploadImage(self, session, provider, image):
        image_filename = self._getStoredImages(image.name)
        if not image_filename:
        ...
        try:
            snap_image = session.createSnapshotImage(
                provider_name=provider, image_name=image.name)

            t = DiskImageUpdater(self, self.config.providers[provider], image,
                                 snap_image.id, image_filename[0])
            t.start()
            ...
            self.log.exception(
                "Could not upload image %s on %s", image.name, provider)
