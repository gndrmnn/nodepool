...
#  AllocationHistory.record_grant().  All the sub-allocations
#  contribute to tracking the total grants for the parent
#  AllocationRequest.
#
#  When finished requesting grants from all providers,
#  AllocationHistory.grants_done() should be called to store the
...
#  AllocationHistory.get_wait_time will return how many iterations
...
class AllocationHistory(object):
    ...
    def record_request(self, label, amount):
        try:
    ...
    def record_grant(self, label, amount):
        try:
            a = self.current_allocations[label]
            a[1] += amount
        except KeyError:
    ...
    def get_wait_time(self, label):
        ...
        for i in range(0, len(self.past_allocations)):
            a = self.past_allocations[i]
...
class AllocationRequest(object):
    ...
    def __init__(self, name, amount, history=None):
        ...
        self.history.record_request(name, amount)
        self.record_grant = functools.partial(self.history.record_grant,
                                              name)
        self.get_wait_time = functools.partial(self.history.get_wait_time,
                                               name)
...
class AllocationSubRequest(object):
    ...
    def getWaitTime(self):
        return self.request.get_wait_time()
    ...
    def grant(self, amount):
        ...
        if amount > 0:
            grant = AllocationGrant(self.request, self.provider,
                                    amount, self.targets)
            self.request.record_grant(amount)
