...
class FakeOpenStackCloud(object):
    ...
    def _get(self, name_or_id, instance_list):
        self.log.debug("Get %s in %s" % (id, repr(instance_list)))
        for instance in instance_list:
            if instance.name == name_or_id or instance.id == name_or_id:
                return instance
        return None

    def _create(self, instance_list, instance_type=Dummy.INSTANCE, **kw):
    ...
    def create_image(self, **kwargs):
        return self._create(
            self._image_list, instance_type=Dummy.IMAGE, **kwargs)
