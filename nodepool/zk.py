...
class ZooKeeper(object):
    ...
    def storeBuild(self, image, build_data, build_number=None):
        ...
        if build_number is None:
            path = self._retry_create_sequence(build_path, build_data.serialize())
    ...
    def storeImageUpload(self, image, build_number, provider, image_data,
        ...
        if upload_number is None:
            path = self._retry_create_sequence(
                upload_path, image_data.serialize())
    ...
    def storeNodeRequest(self, request, priority="100"):
        ...
        if not request.id:
            ...
            path = self._retry_create_sequence(path, request.serialize())
    ...
    def storeNode(self, node):
        ...
        if not node.id:
            ...
            path = self._retry_create_sequence(node_path, node.serialize())
