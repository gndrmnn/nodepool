...
class Launcher(Serializable):
    ...
    Guarantees that supported_labels will always be sorted.
    '''
    ...
    def supported_labels(self):
        return sorted(self._supported_labels)

    @supported_labels.setter
    def supported_labels(self, value):
        if not isinstance(value, list):
            raise TypeError("'supported_labels' attribute must be a list")
        self._supported_labels = sorted(value)

    def toDict(self):
        d = {}
        d['id'] = self.id
        d['supported_labels'] = self.supported_labels
        return d
    ...
    def fromDict(d):
        obj = Launcher()
        obj.id = d.get('id')
        obj.supported_labels = d.get('supported_labels', [])
        return obj
...
class ZooKeeper(object):
    ...
    def registerLauncher(self, launcher):
        ...
        if self.client.exists(path):
            ...
            if obj.supported_labels != launcher.supported_labels:
