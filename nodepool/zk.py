...
class ZooKeeper(object):
    ...
    def getNodeRequest(self, request, cached=False):
        ...
        '''
        if cached:
            node_request = self._cached_node_requests.get(request)
            if node_request:
                return node_request
    ...
    def updateNodeRequest(self, request):
        '''
        Update the data of a ndoe request object in-place
        ...
        try:
            ...
            # TODO(tobiash): return or raise here?
            return
    ...
    def getNode(self, node, cached=False):
        ...
        '''
        if cached:
            node = self._cached_nodes.get(node)
            if node:
                return node
    ...
    def nodeCacheListener(self, event):
        ...
        if event.event_type in (TreeEvent.NODE_ADDED, TreeEvent.NODE_UPDATED):
            node = Node.fromDict(
                self._bytesToDict(event.event_data.data), node_id)
            node.stat = event.event_data.stat

            # Note we intentionally don't update the cached node inplace but
            # replace it in order to not provoke any data races if a different
            # thread is doing something with a node it already got from the
            # cache.
    ...
    def requestCacheListener(self, event):
        ...
        if event.event_type in (TreeEvent.NODE_ADDED, TreeEvent.NODE_UPDATED):
            request = NodeRequest.fromDict(
                self._bytesToDict(event.event_data.data), request_id)
