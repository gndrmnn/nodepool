...
class ZooKeeper(object):
    ...
    def get_max_build_id(self, image):
        ...
        '''
        path = self._image_builds_path(image)
    ...
    def get_image_upload(self, image, build_number, provider,
        ...
        if upload_number is None:
            upload_number = self.get_max_image_upload_id(image, build_number,
                                                         provider)
    ...
    def store_image_upload(self, image, build_number, provider, image_data):
        '''
        ...
        build_path = self._image_builds_path(image)
        ...
        try:
            ...
            # Get a new upload ID
            next_id = self.get_max_image_upload_id(image, build_number,
                                                   provider) + 1

            path = path + "/%s" % next_id
            self.client.create(path, self._dict_to_str(image_data))
        except kze.KazooException as e:
            raise npe.ZKException("Storing image data failed: %s" % e)
