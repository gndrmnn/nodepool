...
class ProviderManager(TaskManager):
    ...
    def deleteKeypair(self, name):
        return self._client.delete_keypair(name)
    ...
    def createServer(self, name, min_ram, image_id=None, image_name=None,
        ...
        return self._client._expand_server_vars(self._client.create_server(
            ...
            if (last_status != status):
                self.log.debug(
                    'Status of {type} in {provider} {id}: {status}'.format(
                        ...
                        status=status))
            last_status = status
            if status in ['READY', 'ACTIVE', 'active', 'ERROR']:
                return resource
    ...
    def waitForServer(self, server_id, timeout=3600):
        ...
        server_with_ip = self._client.add_ips_to_server(
            server, ip_pool=self.provider.pool,
            wait=True, timeout=timeout)
        return self._client._expand_server_vars(server_with_ip)
    ...
    def createImage(self, server_id, image_name, meta):
        return self._client.create_image_snapshot(
            name=image_name, server=server_id, **meta)
    ...
    def listServers(self, cache=True):
        if (not cache or
            ...
            if self._servers_lock.acquire(False):
                try:
                    self._servers = [
                        self._client._expand_server_vars(server) for server in
    ...
    def cleanupServer(self, server_id):
        ...
        if server:
            ...
            fip = shade.meta.get_server_ip(server, ext_tag='floating')
            if fip:
                self._client.delete_floating_ip(fip['id'])
