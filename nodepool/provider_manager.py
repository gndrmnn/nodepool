...
class ProviderManager(TaskManager):
    ...
    def waitForServer(self, server, timeout=3600):
        with shade_inner_exceptions():
            # Use of shade.meta.add_server_interface here can go away
            # once shade > 1.6.1 has been released with the fix, but this
            # should work aroud the problem for now
            result = shade.meta.add_server_interface(
                self._client, self._client.wait_for_server(
