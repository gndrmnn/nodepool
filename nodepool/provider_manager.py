...
class UploadImageTask(Task):
    def main(self, client):
        image = client.glance.images.create(name=self.args['image_name'], is_public=False)
        image.update(data=open(args['filename'], 'rb'))
...
class ProviderManager(TaskManager):
    ...
    def _getClient(self):
        ...
        if self.provider.service_name:
            glance_kwargs['service_name'] = self.provider.service_name
        ...
        nova = novaclient.client.Client('1.1', self.provider.username,
    ...
    def uploadImage(self, image_name, filename):
        return self.submitTask(UploadImageTask(image_name=image_name,
                                               filename=filename))
