...
class ProviderManager(TaskManager):
    ...
    def deleteImage(self, name):
        ...
        with shade_inner_exceptions():
            result = self._client.delete_image(name)
        return result
    ...
    def deleteKeypair(self, name):
        with shade_inner_exceptions():
            result = self._client.delete_keypair(name=name)
        return result
    ...
    def createServer(self, name, min_ram, image_id=None, image_name=None,
        ...
        meta = dict(nodepool_provider_name=self.provider.name)
        group_list = [self.provider.name]
        if nodepool_node_id:
            meta['nodepool_node_id'] = str(nodepool_node_id)
        if nodepool_snapshot_image_id:
            meta['nodepool_snapshot_image_id'] = str(nodepool_snapshot_image_id)
        if nodepool_image_name:
            meta['nodepool_image_name'] = nodepool_image_name
            group_list.append(nodepool_image_name)
        meta['groups'] = ','.join(group_list)
        create_args['meta'] = meta
        self.log.warning("In create server")

        with shade_inner_exceptions():
            self.log.warning("i enter")
            result = self._client.create_server(wait=False, **create_args)
            self.log.warning("after create server")
        return result

    def getServer(self, server_id):
        with shade_inner_exceptions():
            result = self._client.get_server(server_id)
        return result

    def waitForServer(self, server, timeout=3600):
        self.log.debug("I wait for server")
        self.log.debug(self._client)
        with shade_inner_exceptions():
            result = self._client.wait_for_server(
                server=server, auto_ip=True, reuse=False,
                wait=True, timeout=timeout)
            self.log.debug("after wait")
            self.log.debug(vars(server))
        return result
    ...
    def createImage(self, server, image_name, meta):
        with shade_inner_exceptions():
            result = self._client.create_image_snapshot(
                image_name, server=server, metadata=meta)
        return result

    def getImage(self, image_id):
        with shade_inner_exceptions():
            result = self._client.get_image(image_id)
        return result
    ...
    def listFlavors(self):
        with shade_inner_exceptions():
            result = self._client.list_flavors()
        return result
    ...
    def deleteServer(self, server_id):
        with shade_inner_exceptions():
            result = self._client.delete_server(server_id, delete_ips=True)
        return result
