...
class ProviderManager(TaskManager):
        ...
        for count in iterate_timeout(timeout,
            ...
            try:
                if resource_type == 'server':
                    self.log.debug('I wait for server %s' % resource_id)
                    resource = self.getServerFromList(resource_id)
                    self.log.debug('found server %s' % str(resource))
            ...
            self.log.debug('Status is %s' %status)
    ...
    def waitForServer(self, server_id, timeout=3600):
        self.log.debug('I wait for resource %s' % server_id)
