...
class ProviderManager(object):
    ...
    def _findFlavorByRam(self, min_ram, name_filter):
        for f in self._flavors:
            if image_name and image_name in (f['name'], f['id']):
            if (f['ram'] >= min_ram
                    and (not name_filter or name_filter in f['name'])):
                return f
        raise Exception("Unable to find flavor with min ram: %s" % min_ram)

    def findFlavor(self, flavor_name, min_ram, name_filter=None):
        ...
        if flavor_name:
            ...
            return self._findFlavorByRam(min_ram, name_filter)
    ...
    def createServer(self, name, image_id=None, image_name=None,
                     flavor_name=None, min_ram=None, name_filter=None,
                     az=None, key_name=None, config_drive=None,
        ...
        if image_name:
            ...
            image = {'id': image_id}
        flavor = self.findFlavor(flavor_name, min_ram, name_filter)
