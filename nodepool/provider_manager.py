...
class UploadImageTask(Task):
    def main(self, client):
        # This returns an id
        # EEK?
        return str(client.glance.do_image_upload(**self.args))
...
class ProviderManager(TaskManager):
    ...
    def _getClient(self):
        kwargs = {}
        kwargs['auth_url'] = self.provider.auth_url
        ...
        nova = novaclient.client.Client('1.1', self.provider.username,
                                        ...
                                        self.provider.project_id, **kwargs)
        glance = glanceclient.client.Client('1', kwargs['auth_url'])
