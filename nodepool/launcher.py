...
class CleanupWorker(BaseCleanupWorker):
    ...
    def _cleanupMaxHoldAge(self):
        ...
        for node in held_nodes:
            ...
            if (node.hold_expiration is None and
                self._nodepool.config.max_hold_age <= 0):
                continue
            # Operator provider TTL overrides default setting if lower
            elif (node.hold_expiration is not None and
                  self._nodepool.config.max_hold_age <= 0):
                max_uptime = node.hold_expiration
            else:
                max_uptime = min(
                    node.hold_expiration or self._nodepool.config.max_hold_age,
