...
class CleanupWorker(BaseCleanupWorker):
    ...
    def _failDeclinedRequest(self, zk_conn, req, launchers):
        ...
        that should have declined afterwards was removed from the
        configuration before it had a chance to do so. Therefore we need to
        check for requests that are declined by all currently running
        providers and fail them if no provider is left.
