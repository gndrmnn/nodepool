        ...
        for node in zk_conn.nodeIterator():
            if node.allocated_to == req.id:
                try:
                    ...
                    self.log.warning(
                        "Unable to grab lock to unallocate node %s from "
                ...
                try:
                    zk_conn.storeNode(node)
                    self.log.debug("Unallocated node %s for lost request %s",
                                   node.id, req.id)
                except Exception:
                    self.log.exception(
        ...
        for label_name in ready_nodes:
            ...
            for node in ready_nodes[label_name]:
                ...
                self.log.debug("Node %s exceeds max ready age: %s >= %s",
                               (now - node.state_time), label.max_ready_age)
                ...
                try:
                    ...
                    if node.state == zk.READY and node.allocated_to:
                        ...
                        try:
                            zk_conn.storeNode(node)
                            self.log.debug(
                                "Unallocated node %s with missing request %s",
