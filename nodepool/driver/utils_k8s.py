...
def createApiClientClassWithStatsD(klass):
    ...
    class newClass(klass):
        def __init__(self, provider_name, log, configuration=None,
        ...
        def resourcePathToMetricName(self, resource_path, method):
            ...
            if not matches:
                self.log.error(
                    ...
                    % resource_path)
                return (
                    key + '.' + resource_path.replace('/', '_') + '.' + method
                )
            ...
            if mdict['service_type'] == 'api/v1':
                ...
                sfx = mdict['service_type'].split('/')[1].replace('.', '_')
                key += '.' + sfx
            key += '.%s' % method
            ops = mdict['ops'].split('/')
            ...
            if self._statsd:
                with self._statsd.pipeline() as pipeline:
                    ...
                    if status:
                        ...
                        pipeline.incr(key)
                        if duration > 1000:
                            pipeline.incr(metric_name + '.over_1000')
                    pipeline.incr(metric_name + '.attempted')

        # We should really plug ourselves at __call_api, but name mangling
        # complicates things. Not reporting on async call (which we don't
        # do in the k8s providers anyway, it seems) is an acceptable tradeoff.
        def call_api(self, resource_path, method, path_params=None,
            ...
            try:
                ...
                return_data, status, headers = super().call_api(
    ...
    def __init__(self, provider_name, log, api_client=None):
        ...
        if api_client is not None:
            ...
            if api_client.default_headers:
                header_name = list(api_client.default_headers.keys())[0]
