...
class PodLauncher(NodeLauncher):
    ...
    def launch(self):
        ...
        if self.label.type == "project":
            resource = self.handler.manager.createProject(
                self.node.id, self.handler.pool.name)
        else:
            resource = self.handler.manager.createPod(
                self.node.id, self.handler.pool.name, self.label)

        self.node.state = zk.READY
        self.node.external_id = resource['name']
        # NOTE: resource access token may be encrypted here using scheduler key
