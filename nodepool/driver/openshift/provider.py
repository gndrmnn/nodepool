...
class OpenshiftProvider(Provider):
    ...
    def createProject(self, node, pool):
        name = node.id
        ...
        proj = self.os_client.create_project_request(proj_body)
        ...
        # Wait for the openshift token to be created
        for retry in range(30):
        ...
        try:
            self.os_client.create_namespaced_role_binding(namespace, role_body)
        except ValueError:
    ...
    def createPod(self, node, pool, label):
        resource = self.createProject(node, pool)
