...
class ImageFilter:
    ...
    def __init__(self, config):
        ...
        self.private = config.get('private')
        self.tags = config.get('tags')

    def matches(self, image):
        if self.name is not None and fnmatch(image.name, self.name):
            return False
        ...
        if self.id is not None and self.id != image.id:
            return False
        if self.private is not None and self.private != image.private:
            return False
...
class ProviderPool(ConfigPool):
    ...
    def load(self, pool_config, full_config, provider):
        ...
        for label in pool_config.get('labels', []):
            pl = ProviderLabel()
            pl.name = label['name']
            pl.ssh_keys = label.get('ssh-keys', [])
            pl.user_data = label.get('user-data', '')
            pl.pool = self
...
class DigitalOceanProviderConfig(ProviderConfig):
    ...
    def __eq__(self, other):
        if isinstance(other, DigitalOceanProviderConfig):
            return (super().__eq__(other)
    ...
    def manage_images(self):
        # Currently we have no image management for digital ocean. This should
