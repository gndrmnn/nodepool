...
class IBMVPCAdapter(statemachine.Adapter):
    ...
    def __init__(self, provider_config):
        ...
        self.subnets = {}
        self._getSubnets()
        self.subnet_id = self.subnets[self.provider.subnet]['id']
        ...
        if subnet_zone != self.provider.zone:
            raise Exception(f"Subnet {self.provider.subnet} zone "
                            f"{subnet_zone} does not match provider "
                            f"zone {self.provider.zone}")
        self.keys = {}
        self._getKeys()
