...
class DevnestLauncher(NodeLauncher):
    ...
    def _launchLabel(self):
        self.log.debug("Creating devnest resource")
        interface_ip = self.handler.manager.createDevnestNode(
            self.label, self.node)

        self.node.state = zk.READY
        self.node.python_path = "/usr/bin/python"  # Required for ansible
        self.node.connection_type = "ssh"
        self.node.interface_ip = interface_ip
        self.zk.storeNode(self.node)
        self.log.info("Devnest resource %s is ready" % self.node.external_id)
...
class DevnestNodeRequestHandler(NodeRequestHandler):
    log = logging.getLogger("nodepool.driver.Devnest."
                            "DevnestNodeRequestHandler")
    ...
    def launch(self, node):
        ...
        thd = DevnestLauncher(self, node, self.provider, label)
        thd.start()
