...
class AzureProvider(Provider):
    ...
    def _get_compute_client(self):
        return get_client_from_auth_file(
            ComputeManagementClient, auth_path=self.provider.auth_path)

    def _get_network_client(self):
        return get_client_from_auth_file(
            NetworkManagementClient, auth_path=self.provider.auth_path)

    def _get_resource_client(self):
        return get_client_from_auth_file(
            ResourceManagementClient, auth_path=self.provider.auth_path)
    ...
    def listDisks(self):
        return self.compute_client.disks.list_by_resource_group(
            self.resource_group)
    ...
    def _cleanupLeakedNodes(self):
        ...
        for node in self.listNodes():
            if node.tags is None:
                # Nothing to check ownership against, move on
                continue
            if 'nodepool_provider_name' not in node.tags:
                continue
            if node.tags['nodepool_provider_name'] != self.provider.name:
                # Another launcher, sharing this provider but configured
                # with a different name, owns this.
                continue
            if (self.provider.name in deleting_nodes and
                node.id in deleting_nodes[self.provider.name]):
                # Already deleting this node
                continue
            if not self._zk.getNode(node.tags['nodepool_id']):
                self.log.warning(
                    "Marking for delete leaked instance %s (%s) in %s "
                    "(unknown node id %s)",
                    node.name, node.id, self.provider.name,
                    node.tags['nodepool_id']
                )
                node = zk.Node()
        ...
        for disk in self.listDisks():
            if disk.tags is not None and \
                disk.tags.get('nodepool_id') == vm.tags['nodepool_id']:
                    async_disk_delete = self.compute_client.disks.delete(
