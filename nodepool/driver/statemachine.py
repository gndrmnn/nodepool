    ...
    def runStateMachine(self):
        ...
        try:
            ...
            if (state_machine.complete and self.nodescan_request
                ...
                keys = self.nodescan_request.keys
                if keys:
                    node.host_keys = keys
...
class NodescanWorker:
    ...
    def _advance(self, request, socket_ready):
        ...
        if request.state != old_state:
            request.log.debug(
                ...
                "from %s to %s",
                request.node.id, old_state, request.state)
    ...
    def registerDescriptor(self, fd):
        self.poll.register(
            fd, [select.EPOLLOUT, select.EPOLLERR, select.EPOLLONESHOT])
    ...
    def run(self):
        ...
        while self._running:
            ...
            for request in self.requests:
                try:
                    socket_ready = (request.socket and request.socket in ready)
                    self._advance(request, socket_ready)
                except Exception:
                    request.log.exception("Error in nodescan worker:")
                    request.fail()
                if request.complete:
