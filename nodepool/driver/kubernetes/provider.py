...
from nodepool.driver.utils import NodeDeleter, QuotaInformation, QuotaSupport
...
class KubernetesProvider(Provider, QuotaSupport):
    ...
    def cleanupLeakedResources(self):
        ...
        for server in self.listNodes():
            ...
            if not self._zk.getNode(meta["nodepool_node_id"]):
                self.log.warning(
                    "Deleting leaked instance %s (%s) in %s " "(unknown node id %s)",
                    server.name,
                    server.id,
                    self.provider.name,
                ...
                if self._statsd:
                    key = "nodepool.provider.%s.leaked.nodes" % self.provider.name
    ...
    def createNamespace(self, node, pool, label, request, restricted_access=False):
        name = node.id
        ...
        secret_body = {
            ...
            "metadata": {
                "name": user,
                "annotations": {"kubernetes.io/service-account.name": user},
            },
        }
        ...
        if not token or not ca_crt:
            raise exceptions.LaunchNodepoolException(
                "%s: couldn't find token for secret %s" % (namespace, secret)
            )
        ...
        if restricted_access:
            ...
            role_body = {
                ...
                "rules": [
                    ...
                    {"apiGroups": [""], "resources": ["pods/exec"], "verbs": all_verbs},
                    {"apiGroups": [""], "resources": ["pods/logs"], "verbs": all_verbs},
                    {
                        "apiGroups": [""],
        ...
        self.rbac_client.create_namespaced_role_binding(namespace, role_binding_body)
    ...
    def createPod(self, node, pool, label, request):
        ...
        resource = self.createNamespace(
            node, pool, label, request, restricted_access=True
        )
        ...
        if retry == 299:
            raise exceptions.LaunchNodepoolException(
                "%s: pod failed to initialize (%s)" % (namespace, pod.status.phase)
            )
    ...
    def getPodBodyNodepool(self, node, pool, label, request):
        ...
        spec_body = {"containers": [container_body]}
    ...
    def getProviderLimits(self):
        ...
        return QuotaInformation(
            cores=math.inf, instances=math.inf, ram=math.inf, default=math.inf
        )
