...
class GcloudInstanceLauncher(NodeLauncher):
    ...
    def launch(self):
        ...
        while attempts <= self.retries:
            try:
                operation = self.handler.manager.createInstance(
                    hostname, self.label,
        ...
        self.log.debug("Create instance operation %s", repr(operation))
        ...
        while time.monotonic() - boot_start < self.boot_timeout:
            try:
                if self.handler.manager.checkOperation(operation['name']):
                    break
            except Exception:
        ...
        access = iface['accessConfigs'][0]
        server_ip = access['natIP']
...
class GcloudNodeRequestHandler(NodeRequestHandler):
    log = logging.getLogger("nodepool.driver.gcloud."
                            "GcloudNodeRequestHandler")
    ...
    def launch(self, node):
        ...
        thd = GcloudInstanceLauncher(self, node, self.provider, label)
        thd.start()
