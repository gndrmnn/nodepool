...
class FakeProviderImageTestFail(FakeOpenStackCloud):
    ...
    def _filename(self, name):
        if self.image_traces_path and name:
            image_name = '-'.join([name, 'trace'])
            return os.path.join(self.image_traces_path, image_name)
        else:
            return None

    def create_image(self, **kwargs):
        image_file = self._filename(kwargs.get('image_name', None))
        if image_file:
            with open(image_file, 'a'):
                pass
        return super(FakeProviderImageTestFail, self).create_image(**kwargs)
    ...
    def delete_image(self, **kwargs):
        ...
        return super(FakeProviderImageTestFail, self).delete_image(**kwargs)
