...
class OpenContainerNodeRequestHandler(NodeRequestHandler):
    ...
    def run_handler(self):
        ...
        if label:
            self.log.debug("Starting container %s to %s" % (label.config,
                                                            self.request))
            hostid = "%s-%s" % (label.name, self.request.id)
            client, port = self.start_container(label.config, hostid)
            node = zk.Node()
            node.state = zk.READY
    ...
    def start_container(self, config, hostid):
        ...
        if config == "host":
            cmds.append("umount %s/rootfs" % bundle_path)
            cmds.append("mount -o bind,ro --make-private / %s/rootfs" %
                        bundle_path)
