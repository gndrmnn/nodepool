...
class StaticNodeProvider(Provider, QuotaSupport):
    ...
    def getRegisteredNodes(self):
        ...
        for pool in self.provider.pools.values():
            for static_node in pool.nodes:
                self.log.debug("have node %s", static_node)
    ...
    def cleanupLeakedResources(self):
        ...
        with self._register_lock:
            ...
            # De-register nodes to synchronize with our configuration.
            # This case covers any registered nodes that no longer
            # appear in the config, but which may have been added back
            # by a previous provider while stopping.
            for node_tuple, nodes in self._node_slots.items():
                if node_tuple not in static_nodes:
                    for node in nodes:
                        try:
                            ...
                            self.log.exception(
                                "Couldn't deregister static node:")
                            continue
