...
class NodeLauncher(threading.Thread, stats.StatsReporter):
    ...
    def _launchNode(self):
        if self._label.diskimage:
            ...
            config_drive = self.label._cloud_image.config_drive

            image_external = self.label._cloud_image.external
            image_id = self.label._cloud_image.name
            image_name = self.label._cloud_image.name
