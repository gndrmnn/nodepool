...
class OpenStackNodeRequestHandler(NodeRequestHandler):
    ...
    def _checkRemainingQuota(self, ntype):
        ...
        cloud_quota = self.manager.estimatedNodepoolQuota(self.zk)
        ...
        pool_quota = QuotaInformation(cores=self.pool.max_cores,
                                      ...
                                      default=math.inf)
        pool_quota.subtract(self.manager.estimatedNodepoolQuotaUsed(self.zk, self.pool))
        pool_quota.subtract(needed_quota)
        self.log.debug("Predicted remaining pool quota: %s", pool_quota)

        return pool_quota.non_negative()

    def _checkProviderQuota(self, node_types):
    ...
    def _waitForNodeSet(self):
        ...
        for ntype in needed_types:
            ...
            if not got_a_node:
                ...
                if not self._checkRemainingQuota(ntype):
                    if not self.paused:
                        self.log.debug(
    ...
    def run_handler(self):
        ...
        elif not self._checkProviderQuota(self.request.node_types):
