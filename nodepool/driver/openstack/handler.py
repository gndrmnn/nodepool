...
class NodeLauncher(threading.Thread, stats.StatsReporter):
    ...
    def _launchNode(self):
        if self._label.diskimage:
            ...
            config_drive = self._label._cloud_image.config_drive

            image_external = self._label._cloud_image.external
            image_id = self._label._cloud_image.name
            image_name = self._label._cloud_image.name
