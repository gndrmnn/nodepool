...
class AwsDescribeQueue:
    ...
    def startDescribeThread(self):
        ...
        """
        if self.describe_thread.is_alive():
    ...
    def addNewInstance(self, instance_id):
        ...
        if not self.describe_thread.is_alive():
            self.log.debug("describe thread not alive")
            self.startDescribeThread()
