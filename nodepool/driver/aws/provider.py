...
class AwsProvider(Provider):
    ...
    def getLatestImageIdByFilters(self, image_filters):
        try:
            ...
            return image.get("ImageId", None)
        except IndexError:
            self.log.warning(
                "Provider %s is configured to match AMI by filters %s"
                " but no matching images were found."
                % (self.provider.name, image_filters)
            )
            return None

    def getImage(self, cloud_image):
        image_id = cloud_image.external_name

        if cloud_image.image_filters is not None:
            by_filters = self.getLatestImageIdByFilters(
                cloud_image.image_filters
            )
            # If no results by filter, fall back to external_name
            image_id = (
                by_filters if by_filters is not None else image_id
            )
