...
class AwsInstance(statemachine.Instance):
    def __init__(self, provider, instance, host, quota):
        ...
        self.az = None
        self.quota = quota
        self.instance_type = instance['InstanceType']
    ...
    def advance(self):
        ...
        if self.state == self.INSTANCE_CREATING_SUBMIT:
            ...
            if not self.label.instance_type:
                # debug only
                self.log.debug(
                    "Fleet API instance type: %s", instance['InstanceType'])
                instance_type = instance['InstanceType']
            else:
                instance_type = self.label.instance_type

            self.quota = self.adapter.getQuotaForLabel(
        ...
        if self.state == self.COMPLETE:
            self.complete = True
            self.log.debug("self.instance['InstanceType']: %s",
...
class AwsAdapter(statemachine.Adapter):
    ...
    def getQuotaForLabel(self, label, instance_type=None):
        ...
        if label.dedicated_host:
            quota = self._getQuotaForHostType(
                label.instance_type)
        # If fleet is configured, we do not know quotas
        elif label.fleet and not instance_type:
            quota = QuotaInformation(instances=1)
        else:
            quota = self._getQuotaForInstanceType(
                (label.instance_type or instance_type),
                SPOT if label.use_spot else ON_DEMAND)
        if label.volume_type:
    ...
    def _createFleet(self, label, image_id, tags, hostname, log):
        ...
        with self.rate_limiter(log.debug, "Created fleet"):
            ...
            # for debugging
            log.debug("EC2 describe instances result: %s",
