...
class AwsAdapter(statemachine.Adapter):
    ...
    def _deleteSnapshot(self, external_id):
        ...
        if snap['VolumeId'] == 'vol-ffffffff':
            ...
            for task in self._listImportSnapshotTasks():
                task_snapshot_id = task['SnapshotTaskDetail'].get('SnapshotId')
                if task_snapshot_id == external_id:
                    if task['SnapshotTaskDetail']['Status'].lower() in (
                        'completed', 'deleted'):
                        break
                    else:
                        self.log.debug(
                            "Snapshot %s not deleted because import-task "
                            "%s is not completed",
                            external_id, task['ImportTaskId'])
                        return None
