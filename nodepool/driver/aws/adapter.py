...
class AwsAdapter(statemachine.Adapter):
    ...
    def __init__(self, provider_config):
        ...
        launch_tempalte_prefix = 'nodepool-launch-template'
        exising_templates = []
        name_filter = {
            'Name': 'launch-template-name',
            'Values': [f'{launch_tempalte_prefix}-*'],
        }
        ...
        with self.non_mutating_rate_limiter:
            for page in paginator.paginate(Filters=[name_filter]):
                for template in page['LaunchTemplates']:
                    exising_templates.append(template['LaunchTemplateName'])
        ...
        for pool_name, pool in provider_config.pools.items():
            for label_name, label in pool.labels.items():
                ...
                template_name = (f'{launch_tempalte_prefix}-'
                                 f'{label.volume_type}-{label.volume_size}-'
                                 f'{label.iops}-{label.throughput}')
                if template_name in exising_templates:
                    self.log.debug(
                        f'launch template {template_name} already exist')
                    continue

                self.log.debug(f'Creating launch template {template_name}')
                ebs_settings = {
                ...
                try:
                    ...
                    self.log.debug(f'launch template {template_name} created')
                except Exception as e:
                    self.log.debug(
                        f'Could not create launch template {template_name}')
