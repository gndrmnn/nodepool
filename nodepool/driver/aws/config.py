    ...
    def __eq__(self, other):
        if isinstance(other, ProviderLabel):
            ...
            return (other.name == self.name and
                    ...
                    other.volume_type == self.volume_type)
        return False
...
class ProviderPool(ConfigPool):
    ...
    def load(self, pool_config, full_config, provider):
        ...
        self.ignore_provider_quota = pool_config.get('ignore-provider-quota', False)
        ...
        self.host_key_checking = bool(pool_config.get('host-key-checking', True))
        ...
        for label in pool_config.get('labels', []):
            ...
            if cloud_image_name:
                cloud_image = self.provider.cloud_images.get(cloud_image_name, None)
                if not cloud_image:
                    raise ValueError(
    ...
    def __eq__(self, other):
        if isinstance(other, ProviderPool):
            ...
            return (super().__eq__(other) and
                    ...
                    other.availability_zone == self.availability_zone and
                    other.subnet_id == self.subnet_id and
                    other.security_group_id == self.security_group_id and
...
class AwsProviderConfig(ProviderConfig):
    ...
    def __eq__(self, other):
        if isinstance(other, AwsProviderConfig):
            return (super().__eq__(other) and
                    other.profile_name == self.profile_name and
                    other.region_name == self.region_name and
                    other.pools == self.pools and
                    other.rate == self.rate and
                    other.boot_timeout == self.boot_timeout and
                    other.launch_retries == self.launch_retries and
