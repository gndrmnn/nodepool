...
class AwsProviderConfig(ProviderConfig):
    ...
    def load(self, config):
        ...
        vmimport = self.provider.get('vmimport')
        if vmimport:
            self.s3_image_bucket = vmimport['bucket']
            self.s3_image_basedir = vmimport.get('basedir')
            self.ec2_vm_import_role = vmimport.get('role', 'vmimport')
