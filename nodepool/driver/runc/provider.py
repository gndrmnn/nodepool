...
class RuncProvider(Provider):
    ...
    def __init__(self, provider, *args):
        ...
        for pool in self.pools.values():
            # Retro compatibility
            if provider.hypervisor:
                hostname = provider.hypervisor
            else:
                hostname = pool.name
    ...
    def run_ansible(self, playbook, pool, extra_vars=[]):
        # TODO: replace this by a host-key provider setting
    ...
    def listNodes(self):
        ...
        for pool in self.pools.values():
            for server_name in pool.containers:
    ...
    def cleanupNode(self, server_id):
        ...
        server_pool = None
        for pool in self.pools.values():
            if server_id in pool.containers:
                server_pool = pool
                break
        if not server_pool:
        ...
        pool.containers.remove(server_id)
