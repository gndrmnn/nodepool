...
class RuncNodeRequestHandler(NodeRequestHandler):
    ...
    def hasRemainingQuota(self, ntype):
        max_servers = self.provider.pools[self.pool.name].max_servers
        # This is supposedly a light-weight container job which does not
        # consume any RAM and CPU resources and little disk space when already
        # being deleted, so let's parallelize a bit.
        allocated_nodes = sum(1 for node in self.manager._zk.nodeIterator()
                              if node.pool == self.pool.name and
                              node.state != zk.DELETING)
        if max_servers is None or allocated_nodes < max_servers:
            return True
        return False

    def hasProviderQuota(self, ntypes):
        max_servers = self.provider.pools[self.pool.name].max_servers
        if max_servers is None or len(ntypes) <= max_servers:
            return True
        return False
