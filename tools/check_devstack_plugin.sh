...
function showserver {
    ...
    auth_url=$(grep auth_url /etc/openstack/clouds.yaml | head -1 | awk {'print $2'})
    EXPECTED=$(mktemp)
    RESULT=$(mktemp)
    cat /opt/stack/devstack/stackrc || true
    cat /opt/stack/devstack/openrc || true
    source /opt/stack/devstack/openrc admin admin

    nova --os-auth-url $auth_url --os-cacert /opt/stack/data/ca-bundle.pem \
    show $node_id | grep -Eo "user_data[ ]+.*|[ ]*$" | awk {'print $3'} |\
