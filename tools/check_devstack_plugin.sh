...
NODEPOOL_PAUSE_GENTOO_SYSTEMD_DIB=${NODEPOOL_PAUSE_GENTOO_SYSTEMD_DIB:-True}
...
if [ ${NODEPOOL_PAUSE_GENTOO_SYSTEMD,,} = 'false' ]; then
    # check that image built
    waitforimage gentoo-systemd
    # check image was bootable
    waitfornode gentoo-systemd
    # check ssh for root user
    sshintonode gentoo-systemd
fi
