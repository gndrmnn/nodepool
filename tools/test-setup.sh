...
echo 'JVMFLAGS="-Djava.security.auth.login.config=/etc/zookeeper/conf/auth.conf"' | \
    sudo tee -a /etc/default/zookeeper
echo 'JAVA_OPTS="-Djava.security.auth.login.config=/etc/zookeeper/conf/auth.conf"' | \
    sudo tee -a /etc/default/zookeeper
