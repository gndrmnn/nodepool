...
class MyZMQ(object):
    def __init__(self):
        ...
        self.zsocket = self.context.socket(zmq.PUB)
        self.zsocket.bind("tcp://*:8881")

    def send_start(self, nodename):
        topic = 'onStarted'
        data = {"name":"test","url":"job/test/","build":{"full_url":"http://localhost:8080/job/test/1/","number":1,"phase":"STARTED","url":"job/test/1/","node_name":nodename}}
        self.zsocket.send("%s %s" % (topic, json.dumps(data)))

    def send_stop(self, nodename):
        topic = 'onFinalized'
        data = {"name":"test","url":"job/test/","build":{"full_url":"http://localhost:8080/job/test/1/","number":1,"phase":"FINISHED","status":"SUCCESS","url":"job/test/1/","node_name":nodename, "parameters":{"BASE_LOG_PATH":"05/60105/3/gate","LOG_PATH":"05/60105/3/gate/gate-tempest-dsvm-postgres-full/bf0f215","OFFLINE_NODE_WHEN_COMPLETE":"1","ZUUL_BRANCH":"master","ZUUL_CHANGE":"60105","ZUUL_CHANGE_IDS":"60105,3","ZUUL_CHANGES":"openstack/cinder:master:refs/changes/05/60105/3","ZUUL_COMMIT":"ccd02fce4148d5ac2b3e1e68532b55eb5c1c356d","ZUUL_PATCHSET":"3","ZUUL_PIPELINE":"gate","ZUUL_PROJECT":"openstack/cinder","ZUUL_REF":"refs/zuul/master/Z6726d84e57a04ec79585b895ace08f7e","ZUUL_URL":"http://zuul.openstack.org/p","ZUUL_UUID":"bf0f21577026492a985ca98a9ea14cc1"}}}
        self.zsocket.send("%s %s" % (topic, json.dumps(data)))
...
def main():
    ...
    pool = nodepool.NodePool(sys.argv[1])
    ...
    while True:
        with pool.getDB().getSession() as session:
            for node in session.getNodes():
                if node.state == 2:
                    zmq.send_start(node.nodename)
                    completion_times[node.nodename] = time.time()+5
                if node.state == 3:
                    end = completion_times.get(node.nodename)
                    if end and time.time() > end:
                        zmq.send_stop(node.nodename)
                        del completion_times[node.nodename]
                        run += 1
        time.sleep(1)
        print 'elapsed: %s, run: %s' % (int(time.time()-start), run)
