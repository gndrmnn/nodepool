...
class Server(object):
    ...
    def advance(self, app, elapsed):
        if self.state == 'init':
            ...
            app.queue2.append(Task(self, 'create', 5.0))
            self.state = 'create'
            return
        if self.state == 'create':
            app.queue.append(Task(self, 'server-list', 4.0))
            self.state = 'server-list'
            return
        if self.state =='server-list':
            app.queue.append(Task(self, 'fip-create', 8.0))
            self.state = 'fip-create'
            return
        if self.state =='fip-create':
            app.queue.append(Task(self, 'fip-list', 2.0))
            self.state = 'fip-list'
            return
        if self.state =='fip-list':
            app.queue.append(Task(self, 'fip-attach', 8.0))
